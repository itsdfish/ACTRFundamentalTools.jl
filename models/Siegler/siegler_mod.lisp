;(setf file-name "/home/dfish/.julia/dev/ACTRTutorial/Tutorial_Models/Practice Problems/Siegler RT/Lisp_Data.csv")
;(load "/home/dfish/.julia/dev/ACTRTutorial/actr6/load-act-r-6.lisp")
;(load "/home/dfish/.julia/dev/ACTRTutorial/Tutorial_Models/Practice Problems/Siegler RT/siegler_mod.lisp")
...
(defun test-fact (arg1 arg2)
  ...
  (run 30)
  (setf rt (/(- *response-time* start-time) 1000.0))
  (setf output (list arg1 arg2 *response* rt))
  output)
...
(defun generate-recovery-data (n-reps n-blocks path)
  (sgp :seed (49065 124))
  (let ((responses (list)) (file-name) (all-parms (list (list "mp" "tau" "s"))) (parms (list)))
    (dotimes (i n-reps)
      (setf responses (list))
      (setf *mp* (sample 5.0 20.0))
      (setf *tau* (sample -1.0 1.0))
      (setf *s-scalar* (sample .1 .8))
      (setf parms (list *mp* *tau* *s-scalar*))
      (setf all-parms (append all-parms (list parms)))
      (dotimes (b n-blocks)
        (setf responses (append responses (do-siegler-set))))
...
(define-model siegler
  ...
  (sgp-fct (list :rt *tau* :mp *mp* :esc t :v nil :act nil :ans *s-scalar*))
...
;;add a production to respond with retrieval failure.
;;;note that there is no harvest production when a retrieval failure occurs
(P harvest-answer
   =retrieval>
      ISA         plus-fact
      sum         =number
