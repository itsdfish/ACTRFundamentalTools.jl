...
ggplot(g.gantt, aes(date, Activity, color = Project.element, group=Item)) +
  ...
  theme(text = element_text(size=6),axis.text=element_text(size=6),legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
 scale_colour_economist()
#Sized to look good at 300 by 200 pixels
ggsave("TimeCourse.eps",width = 66, height = 40, units = "mm")
