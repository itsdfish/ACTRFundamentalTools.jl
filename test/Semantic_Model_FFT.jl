...
@safetestset "Likelihood" begin
    ...
    blc = 1.0
    δ = 1.0
    parms = (noise = true, τ = 0.0, s = .3, mmp = true)
    stimuli = get_stimuli()
    n_reps = 2_000
...
@safetestset "Semantic FFT Zero Chains" begin
    ...
    @test .15 > maximum(abs.(exact_density_yes .- sim_density_yes))
    ...
    @test .05 > maximum(abs.(exact_density_no .- sim_density_no))
end
...
@safetestset "Semantic FFT all no one Chain" begin
    ...
    @test .10 > maximum(abs.(exact_density_no .- sim_density_no))
end
